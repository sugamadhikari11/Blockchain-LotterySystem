...
contract Lottery {
    ...
    function endLottery(uint256 roomId) external onlyOwner(roomId) roomActive(roomId) {
        ...
        for (uint256 i = 0; i < rooms[roomId].players.length; i++) {
            ...
            rooms[roomId].hasEntered[rooms[roomId].players[i]] = false; // Reset participation status

        }

        delete rooms[roomId].players; // Clear the players list

        rooms[roomId].totalBalance = 0; // Reset total balance

        // The room remains active for the next round
